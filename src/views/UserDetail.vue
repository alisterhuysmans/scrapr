...
<template>
    <div>
        <h1 style="text-align: center">User Detail for {{ username }}</h1>
        <div v-if="loading">Loading...</div>
        <div class="user" v-else>
            <UserAvatar
                :avatarUrl="
                    'https://scrapifier.com/photoprofile/' + user.avatar
                "
            ></UserAvatar>
            <UserInfo :user="user"></UserInfo>
            <UserStatsChart
                v-if="followerGrowthData.length"
                type="bar"
...
export default {
    ...
    methods: {
        ...
        async fetchUserStats() {
            try {
                const response = await fetch(
                    `https://scrapifier.com/api/usersdata/${this.username}`
                );
        ...
        calculateFollowerGrowth(totalFollowers) {
            this.followerGrowthData = totalFollowers.map((value, index) =>
                index > 0 ? value - totalFollowers[index - 1] : 0
            );
            ...
            this.followerGrowthData = this.followerGrowthData.map((value) =>
                value === 0 ? minBarHeight : value
            );
        },
    },
};
